...
def stock_list_by_price(driver):
        ...
        for sosok in range(2):
            for page in range(1, pages+1):
                ...
                for element in table_elements: # 필요한 정보: 종목명, 현재가, 거래량
                    ...
                    info_tuple = info[1], int(info[2].replace(",","")), int(info[9].replace(",",""))

                    if info_tuple[2] == 0: # 거래량이 0이라면. 즉, 거래정지된 종목이라면.
                        continue
                    if info_tuple[1] > maximum_price or info_tuple[1] < minimum_price: # 주가가 원하는 범위 내가 아니라면,
                        continue
